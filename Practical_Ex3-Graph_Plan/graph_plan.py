...
def independent_pair(a1: Action, a2: Action):
    ...
    for add_prop in a1.get_add():
        if a2.is_neg_effect(add_prop):
            return False

    for add_prop in a2.get_add():
        if a1.is_neg_effect(add_prop):
            return False

    # interference: One action deletes the precondition of another
    for pre_prop in a1.get_pre():
        if a2.is_neg_effect(pre_prop):
            return False

    for pre_prop in a2.get_pre():
        if a1.is_neg_effect(pre_prop):
            return False
