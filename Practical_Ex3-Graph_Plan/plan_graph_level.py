...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer: PropositionLayer):
        ...
        for action in all_actions:
            ...
            for pre1 in action_pre:
                for pre2 in action_pre:
                    if (pre1 == pre2) or (not previous_proposition_layer.is_mutex(pre1, pre2)):
    ...
    def update_proposition_layer(self):
        ...
        "*** YOUR CODE HERE ***"
        # find props to add
        added_props = dict()  # dict(prop_name : [producers])
        for action in current_layer_actions:
            for prop in action.get_add():
                if prop in added_props:
                    added_props[prop].append(action)  # add action to prop's producers
                else:
                    added_props[prop] = [action]  # create new item in dict

        # actual adding of the props
        for prop in added_props:
            prop.set_producers(added_props[prop])
...
def mutex_propositions(prop1: Proposition, prop2: Proposition, mutex_actions_list):
    ...
    for prod1 in producers1:
        for prod2 in producers2:
